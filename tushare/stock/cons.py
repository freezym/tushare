...
#mdofiy by Allen @20170417 start:add column 'ticktime','pricechange'
DAY_TRADING_COLUMNS = ['code', 'symbol', 'name', 'changepercent','pricechange',
                       'trade', 'open', 'high', 'low', 'settlement', 'volume', 'turnoverratio',
                       'amount', 'per', 'pb', 'mktcap', 'nmc', 'ticktime']
#mdofiy by Allen @20170417 end
...
#add by Allen @20170420 start:add method _code_to_s_symbol
def _code_to_s_symbol(code):
    """
        生成s_symbol代码标志,新浪接口用
    """
    if code in INDEX_LABELS:
        return INDEX_LIST[code]
    else:
        if len(code) != 6 :
            ...
            return 's_sh%s'%code if code[:1] in ['5', '6', '9'] else 's_sz%s'%code

#add by Allen @20170420 end
